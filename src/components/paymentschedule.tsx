...
function PaymentSchedule() {
  return (
    <div className="portal-card w-full p-6 sm:p-8 lg:p-10">
      <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[var(--color-primary)]">Payment plan</p>
      ...
      <div className="mt-6 space-y-3">
        {payments.map((item) => (
          <div key={item.label} className="portal-card-strong flex flex-wrap items-center justify-between px-4 py-4">
            ...
            <p className="text-sm font-semibold text-[var(--color-primary)]">{item.amount}</p>
          </div>
        ))}
      </div>
    </div>
